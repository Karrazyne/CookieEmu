...
namespace EleReader
{
    public class EleInstance : INotifyPropertyChanged
    {
        ...
        public EleInstance()
        {
            ...
            KarrazyneData = new Dictionary<int, List<EleGraphicalData>>();
        ...
        public static Dictionary<int, List<EleGraphicalData>> KarrazyneData { get; set; }
        ...
        public static EleInstance ReadFromStream(BigEndianReader reader)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                if (instance.Version <= 8 || !instance.m_lazyLoad)
                {
                    ...
                    if(!KarrazyneData.ContainsKey(elem.Id))
                        KarrazyneData.Add(elem.Id, new List<EleGraphicalData>());
                    KarrazyneData[elem.Id].Add(elem);
                }
                else
                {
                    // never go there atm
