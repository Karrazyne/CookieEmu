...
namespace CookieEmu.API.Extensions
{
    public static class SocketExtensions
    {
        ...
        public static Task<int> ReceiveTask(this Socket s, byte[] buffer, int offset, int size, SocketFlags sf = SocketFlags.None) =>
            Task.Factory.FromAsync(s.BeginReceive(buffer, offset, size, sf, null, s), s.EndReceive);

        public static Task<int> SendTask(this Socket s, byte[] buffer, int offset, int size, SocketFlags sf = SocketFlags.None) =>
            Task.Factory.FromAsync(s.BeginSend(buffer, offset, size, sf, null, s), s.EndSend);

        public static Task ConnectTask(this Socket s, EndPoint endPoint) =>
