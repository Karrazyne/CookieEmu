...
namespace D2OSync.D2i
{
    public class FastD2IReader : IDisposable
    {
        ...
        //Gloabal Variables
        private static readonly FastD2I MyD2I = new FastD2I();
        ...
        private static void LoadD2I()
        {
            ...
            {
                //Get the total size
                MyD2I.SizeOfD2I = _br.BaseStream.Length;
                //Get the data size
                MyD2I.SizeOfData = ReadInt();
                //Load the data
                while (_br.BaseStream.Position < MyD2I.SizeOfData)
                {
                ...
                //Load indexes
                while (_br.BaseStream.Position - MyD2I.SizeOfData < MyD2I.SizeOfIndex)
                {
            ...
            if (_isFastLoad == false)
                try
                {
                    if (versionDiacritique)
                    {
                        ...
                        try
                        {
                            pointer = MyD2I.IndexList.First(n => (n.StrKey == myId) & n.DiaExist).DiaIndex;
                        }
                        catch (Exception)
                        {
                            pointer = MyD2I.IndexList.First(n => n.StrKey == myId).StrIndex;
                        }

                        result.Str = MyD2I.DataList.First(m => m.StrIndex == pointer).Str;
                    }
                    else
                    {
                        var pointer = MyD2I.IndexList.First(n => n.StrKey == myId).StrIndex;
                        result.Str = MyD2I.DataList.First(m => m.StrIndex == pointer).Str;
                    }
                }
                catch (Exception)
                ...
                using (_br = new BinaryReader(File.Open(_pather, FileMode.Open, FileAccess.Read, FileShare.ReadWrite)))
                {
                    MyD2I.SizeOfD2I = _br.BaseStream.Length;
                    MyD2I.SizeOfData = ReadInt();
                    _br.BaseStream.Position = MyD2I.SizeOfData;
                    MyD2I.SizeOfIndex = ReadInt();
                    try
                    {
                        while (_br.BaseStream.Position - MyD2I.SizeOfData < MyD2I.SizeOfIndex)
                        {
                            ...
                            {
                                temp.StrKey = temp2;
                                temp.DiaExist = ReadBool();
                                temp.StrIndex = ReadInt();
                                if (temp.DiaExist)
                                    temp.DiaIndex = ReadInt();
                                var pointer = versionDiacritique ? temp.DiaIndex : temp.StrIndex;
        ...
        public string GetUi(string mySearch)
        {
            ...
            using (_br = new BinaryReader(File.Open(_pather, FileMode.Open, FileAccess.Read, FileShare.ReadWrite)))
            {
                ...
                _br.BaseStream.Position = MyD2I.SizeOfData;
                MyD2I.SizeOfIndex = ReadInt();
                _br.BaseStream.Position = _br.BaseStream.Position + MyD2I.SizeOfIndex;
