...
namespace D2OSync.D2o
{
    public class ObjectDataManager : Singleton<ObjectDataManager>
    {
        ...
        public Type Get(Type T, int key)
        {
            if(!readers.ContainsKey(T))
                throw new ArgumentException("Cannot find data corresponding to type : " + T);
            var reader = readers[T];
            return reader.ReadObject(T, key, true, false);
        }
